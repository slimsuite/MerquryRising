...
* An optional read kmer `*.hist.ploidy` file given with `ploidyfile=FILE`.
...
```{r dump1, eval=FALSE, include=FALSE}
# Load libraries
library(tidyverse)
library(ggridges)
library(GGally)
...
settings = list(basefile="merquryrising",   # Prefix for output files (log and plots)
                ...
                pdfwidth=40,pdfheight=0,pdfscale=1,namesize=1,labelsize=1,labels=TRUE,
...
```{r hapkmerclasses, eval=FALSE, include=FALSE}
...
**NOTE:* This needs to be moved to AFTER the tables are loaded, so ploidy can be calculated from the hist files directly.
...
if(settings$boundary == "calculate"){
  ...
  ploidy$boundary[2] <- (ploidy$depth[2] + settings$diploid) / 2
  ploidy$boundary[3] <- 1.5 * settings$diploid
}
...
densityDiploid <- function(kmerTab,minfreq=5,maxfreq=1000,adjust=1){
  ...
  abline(v=ploidy$depth[2],col="black")
  return(kdiploid)
}
#kdip <- densityDiploid(gdb)
```
...
for(G in adb$G){
  ...
  if(nrow(Tab1) > 0){
    ...
    Tab1 <- gk
    settings$diploid <- densityDiploid(gdb)
  }
}
...
merquryPlot <- function(histTab,ytitle="kmer count shift"){
  dat <- dat %>%
  ...
  p <- ggplot(dat, aes(x = rfreq, y = knum, color = afreq)) +
    ...
    geom_vline(xintercept=settings$diploid, color="grey") +
