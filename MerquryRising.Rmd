...
#i# Generic plot saving function. Adjust height by data rows.
...
p <- ggplot(D$Tab5, aes(x = percentage, y = G, fill = class)) +
  geom_bar(stat = "identity") +
...
for(i in 1:nrow(D$adb)){
  ...
  print(kable(D$adb[i,], "html", caption = D$adb$name[i], align = "l") %>% kable_styling("striped", full_width = F))
  cat(paste('\n\n'))
  if(! pref){
...
#!# Add percentage tables, along with  the reference and difference.

#i# Convert into relative values
#D$Tab4rel <- D$Tab4w  # Converted into relative values.
#D$Tab5rel <- D$Tab5w  # Converted into relative values.
