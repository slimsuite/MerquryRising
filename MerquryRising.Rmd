...
Please check the settings loaded from the config file, below.
...
override <- c('fullrun=FALSE','rdir=.',debug=TRUE)
...
Kmer classes will be assigned based on the read frequency (`rfreq`) and kmer frequency (`kfreq`), ploidy boundaries (see Tutorial and below), and whether the assembly is haploid or diploid.
...
#i# Create and summarise the input data table
adb <- setInputFiles()
if(settings$debug){
...
if(nrow(boundary) > 0){
  kable(boundary, "html", caption = "Ploidy boundaries.", align = "l") %>% kable_styling("striped", full_width = F)
}
...
for(G in adb$G){
  ...
  boundary <- bind_rows(boundary,makeBoundary(dipk,G))
  if(nrow(Tab1) > 0){
...
# Temp ploidy
if(nrow(ploidy) < 1){
  bD <- makeBoundary(settings$diploid)
  ploidy <- tibble(ploidy=c(0,1,2),
                   depth=c(0,bD$haploid,bD$diploid),
                   boundary=c(bD$low,bD$mid,bD$high))
}
